...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, items }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center dark:bg-(--page-dark) justify-between px-4  border-b h-full border-gray-200">
        {sidebarOpen && (
          <h1 className="text-xl font-semibold tracking-wide dark:text-(--text-dark)  dark:bg-(--page-dark) text-gray-700">
            Painel
          </h1>
        )}
        <Button
        ...
        >
          <MenuIcon className="h-5 w-5 dark:text-(--text-dark)  text-gray-600" />
        </Button>
      </div>

      <nav className="flex flex-col dark:bg-(--page-dark) h-full gap-1 p-3 mt-2">
        {items.map((item, index) => (
          <Link
            key={index}
            to={item.href}
            className={cn(
              "flex items-center gap-3 p-2 rounded-xl font-medium dark:bg-(--page-dark) dark:text-(--text-dark) text-gray-700 transition-all duration-200",
              "hover:bg-gray-200 hover:text-gray-900 group"
            )}
          >
            <div className="flex justify-center items-center w-8 h-8 rounded-lg  dark:bg-(--page-dark) dark:text-(--text-dark) bg-gray-100 group-hover:bg-gray-200 transition">
      ...
      <div className="mt-auto border-t dark:bg-(--page-dark) border-gray-200 p-3">
        <button
          onClick={logout}
          className=" flex items-center gap-3 p-2 w-full rounded-xl text-left dark:bg-(--page-dark) dark:text-(--text-dark) text-gray-700 hover:bg-red-50 hover:text-red-600 transition-all duration-200"
        >
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex min-h-screen dark:bg-(--page-dark) bg-gray-50">
